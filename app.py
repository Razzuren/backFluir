...
def process_referral():
    try:
        ...
        json_match = re.search(r'\{.*\}', referral_response.content)
        if not json_match:
            app.logger.error("Failed to find JSON in the response.")
            return jsonify({"error": "Failed to find JSON in the response."}), 500
